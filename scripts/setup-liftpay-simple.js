...
async function setupsplitrSystem() {
  ...
  try {
    ...
    await connection.query(`DROP TRIGGER IF EXISTS trg_merchant_splitr_id`);
    await connection.query(`DROP TRIGGER IF EXISTS trg_buyer_splitr_id`);
    await connection.query(`DROP TRIGGER IF EXISTS trg_loan_splitr_id`);
    await connection.query(`DROP TRIGGER IF EXISTS trg_loan_transaction_splitr_id`);
    await connection.query(`DROP TRIGGER IF EXISTS trg_invoice_splitr_id`);
    await connection.query(`DROP TRIGGER IF EXISTS trg_merchant_transaction_splitr_id`);
    ...
    await connection.query(`
      CREATE TRIGGER trg_invoice_splitr_id
      BEFORE INSERT ON Invoice
      FOR EACH ROW
      BEGIN
        IF NEW.splitrId IS NULL OR NEW.splitrId = '' THEN
          SET NEW.splitrId = generate_splitr_id('LPI');
        END IF;
      END
    `);
    ...
    await connection.query(`
      CREATE TRIGGER trg_merchant_transaction_splitr_id
    ...
    const [testMerchant] = await connection.execute(`SELECT generate_splitr_id('LPM') as id`);
    const [testBuyer] = await connection.execute(`SELECT generate_splitr_id('LPB') as id`);
    const [testLoan] = await connection.execute(`SELECT generate_splitr_id('LPL') as id`);
    const [testLoanTransaction] = await connection.execute(`SELECT generate_splitr_id('LPP') as id`);
    const [testInvoice] = await connection.execute(`SELECT generate_splitr_id('LPI') as id`);
    // const [testMerchantTransaction] = await connection.execute(`SELECT generate_splitr_id('LPMT') as id`);
    ...
    console.log('   LPP  - splitr Payment (Loan Transaction)');
    console.log('   LPI  - splitr Invoice');
    console.log('   LPMT - splitr Merchant Transaction');

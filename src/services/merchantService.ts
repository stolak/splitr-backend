...
const merchantUniqueFields = Object.keys(MERCHANT_UNIQUE_FIELD_LABELS) as MerchantUniqueField[];

const duplicateMerchantFieldMessage = (field: MerchantUniqueField) =>
  `Another merchant exists with provided ${MERCHANT_UNIQUE_FIELD_LABELS[field]}`;

export class MerchantService {
  /**
   * Reject values already registered to another merchant. Every column listed in
   * MERCHANT_UNIQUE_FIELD_LABELS is unique in the database, so this turns the constraint
   * violation into a readable message before Prisma raises it. Soft-deleted merchants still
   * hold on to their values, so they are intentionally not excluded from the lookup.
   */
  private async ensureUniqueFieldsAreAvailable(
    values: Partial<Record<MerchantUniqueField, unknown>>,
    excludeId?: string
  ) {
    const checks = merchantUniqueFields
    ...
    const conflicts = await prisma.merchant.findMany({
      where: {
        ...(excludeId && { id: { not: excludeId } }),
        OR: checks.map(({ field, value }) => ({ [field]: value }) as Prisma.MerchantWhereInput),
      },
      select: merchantUniqueFields.reduce(
        (select, field) => ({ ...select, [field]: true }),
        {} as Record<MerchantUniqueField, true>
      ),
    });

    const taken = checks.find(({ field, value }) =>
      conflicts.some((conflict) => conflict[field] === value)
    );

    if (taken) {
      throw new Error(duplicateMerchantFieldMessage(taken.field));
    }
  }

  /**
   * Translate a unique constraint violation raised between the pre-check and the write
   * (concurrent requests, or a column the caller did not go through the pre-check for)
  ...
  private rethrowDuplicateMerchantField(error: any): never {
    if (error?.code === 'P2002') {
      ...
      const duplicated =
        merchantUniqueFields.find((field) => targets.includes(field)) ??
        merchantUniqueFields.find((field) => targets.some((entry) => entry.includes(field)));

      if (duplicated) {
        throw new Error(duplicateMerchantFieldMessage(duplicated));
      }
    }
    throw error;
  }
